...
try:
    ...
    logging.getLogger("HWR").warning(
                "EnergyScan: PyChooch not available"
            )
...
class BIOMAXContinuousScan(AbstractEnergyScan, HardwareObject):
    ...
    def init(self):
        ...
        self.remote_energy_table = open('/mxn/groups/biomax/amptek/maxlab_macros/remote-energy.dat', 'r')
        ...
        if self.db_connection_hwobj is None:
            logging.getLogger("HWR").warning('BIOMAXEnergyScan: Database hwobj not defined')

        self.transmission_hwobj = self.get_object_by_role("transmission")
        if self.transmission_hwobj is None:
            self.can_scan = False
            logging.getLogger("HWR").warning('BIOMAXEnergyScan: Transmission hwobj not defined')

        self.energy_hwobj = self.get_object_by_role("energy")
        if self.energy_hwobj is None:
            self.can_scan = False
            logging.getLogger("HWR").warning('BIOMAXEnergyScan: Energy hwobj not defined')

        self.beam_info_hwobj = self.get_object_by_role("beam_info")
        if self.beam_info_hwobj is None:
            logging.getLogger("HWR").warning('BIOMAXEnergyScan: Beam Info hwobj not defined')
        ...
        if self.diffractometer_hwobj is None:
            self.can_scan = False
            logging.getLogger("HWR").warning('BIOMAXEnergyScan: Diffractometer hwobj not defined')

        try:
            self.safety_shutter_hwobj =  self.get_object_by_role("safety_shutter")
        except KeyError:
            logging.getLogger("HWR").warning('Energy: error initializing safety shutter')
        ...
        try:
            self.door = PyTango.DeviceProxy('biomax/door/01')
        except:
            self.can_scan = False
            logging.getLogger("HWR").error('Unable to connect to sardana door device')
        try:
            self.pidx = PyTango.DeviceProxy('b311a/ctl/pid-01')
            self.pidx.set_timeout_millis(7000)
            self.pidy = PyTango.DeviceProxy('b311a/ctl/pid-02')
    ...
    def calculate_emission_and_edge_energy(self, element=None, edge=None):
        ...
        logging.getLogger("HWR").info("Absorption edge for %s %s: %f eV; Emission energy: %f eV", element, edge, edge_energy, emission)
    ...
    def run_sardana_macro(self, startEnergy, endEnergy, scan_file, scan_dir):
        ...
        if self.door.state() != PyTango.DevState.ON:
            logging.getLogger("HWR").error('Unable to run scan macro, door is not ready')
            raise RuntimeError('Unable to run scan macro, door is not ready')
        logging.getLogger("HWR").info("Sardana macro: energyContScan_mx3 %s %s %s %s",
            str(startEnergy),
            str(endEnergy),
            scan_file,
            scan_dir)

        self.door.runMacro(['energyContScan_mx3', str(startEnergy), str(endEnergy), scan_file, scan_dir])
        ...
        with gevent.Timeout(60, RuntimeError("Scan macro timeout")):
             while self.door.state() == PyTango.DevState.RUNNING:
                 gevent.sleep(0.2)

    def stop_pid_controllers(self):
        try:
            if (self.pidx.State() == PyTango.DevState.MOVING) and (self.pidy.State() == PyTango.DevState.MOVING):
                self.pidx.StopCtrlLoop()
                self.pidy.StopCtrlLoop()
        except:
            logging.getLogger("HWR").warning("Exception while stopping PID loops, nevertheless data acquisition continues.")


    def start_pid_controllers(self):

        if (self.pidx.State() != PyTango.DevState.MOVING) or (self.pidy.State() != PyTango.DevState.MOVING):

            self.pidx.StartCtrlLoop()
            self.pidy.StartCtrlLoop()

            with gevent.Timeout(10, Exception("Timeout waiting for PID Controllers to be ready")):
                while (self.pidx.State() != PyTango.DevState.MOVING) or (self.pidy.State() != PyTango.DevState.MOVING):
                    gevent.sleep(0.2)
...
#       self.pandabox.Arm() # to be checked its need
    ...
    def prepare_transmission(self, element, edgeElement):
        edge_energy, emission = self.calculate_emission_and_edge_energy(element, edgeElement)
        ...
        try:
            logging.getLogger("HWR").info("Setting prepare transmission energy %s" %str(_energy))
        ...
        try:
            logging.getLogger("HWR").info("Setting start transmission %s" %str(SCAN_TRANSMISSION))
            self.transmission_hwobj.set_value(float(SCAN_TRANSMISSION), wait = True)
        except Exception as ex:
            logging.getLogger("HWR").error('EnergyScan: cannot set scan transmission %s' %ex)
            return False
        ...
        logging.getLogger("HWR").info("Setting start transmission, counts: %s" %str(counts))
        logging.getLogger("HWR").info("Setting start transmission, counts: %s" %str(raw_counts))
        ...
        if counts <= MIN_COUNTS:
            if counts > 0:
                ...
                if transmission > 10:
                    transmission = 10
                    logging.getLogger("HWR").info('Optimal transmission not found, but adjusted to 10%')
                else:
                    logging.getLogger("HWR").info('Optimal transmission found %s' %str(transmission))
            else:
                logging.getLogger("HWR").info('Optimal transmission not found, counts are zero.')
        ...
        try:
            logging.getLogger("HWR").info("Setting transmission for energy scan %s" %str(transmission))
    ...
    def prepare_directory(self, directory):
        if not os.path.isdir(directory):
            ...
            try:
                ...
                logging.getLogger("HWR").error("EnergyScan: error creating directory %s (%s)" % (directory, str(ex)))
                self.emit('energyScanFailed', ("Error creating directory",))
                raise RuntimeError("EnergyScan: error creating directory %s (%s)" % (directory, str(ex)))
        return True
    ...
    def energy_values(self, central_energy):
        ...
        for x in self.Eh:
            if x > Es and x < Ef:
                if x - Es < 30:
                    Es = x + 1
                    logging.getLogger("HWR").warning("Scan between %s and %s to avoid a harmonic jump.", Es, Ef)
                elif Ef - x < 30:
                    Ef = x
                    logging.getLogger("HWR").warning("Scan between %s and %s to avoid a harmonic jump.", Es, Ef)
                else:
                    logging.getLogger("HWR").error("The absorption edge is too close to a harmonic jump.")

        return Es, Ef

    def start_energy_scan(self, element, edgeElement, directory, prefix, \
                 session_id= None, blsample_id= None, cpos=None, exptime= 0.1):
        """
        Descript. :
        """
        ...
        logging.getLogger("HWR").debug("BIOMAXEnergyScan: {}".format(energy_scan_filename))
        if os.path.exists(energy_scan_filename):
            logging.getLogger("HWR").debug("BIOMAXEnergyScan: Aborting execution, File already exists on disk {}".format(energy_scan_filename))
            logging.getLogger("user_level_log").error("BIOMAXEnergyScan: Aborting execution, File already exists on disk {}".format(energy_scan_filename))
            self.scan_command_aborted()
            raise Exception("File already exists on disk {}".format(energy_scan_filename))
        ...
        self.scanInfo = {"sessionId": session_id, "blSampleId": blsample_id,
                         "element": element,"edgeEnergy": edgeElement}
        ...
        try:
            try:
                logging.getLogger("HWR").info("Setting start energy %s" %str(Es/1000))
        ...
        #    self.scan_command_finished(prefix, directory)
            ...
            logging.getLogger("HWR").error('EnergyScan: error in executing energy scan command %s' %ex)
        ...
        logging.getLogger("HWR").info("Setting original energy %s" %str(initial_energy_value))
        self.energy_hwobj.startMoveEnergy(initial_energy_value, check_beam=False)
        logging.getLogger("HWR").info("Setting original transmission %s" %str(initial_transmission_value))
    ...
    def read_raw_data(self, directory, prefix):
        ...
        try:
            if len(self.scan_data) != 0:
                ...
                logging.getLogger("HWR").info("Saving scan data to txt file %s" %txt_path)
    ...
    def scan_command_finished(self, prefix, directory):
        ...
        with cleanup(self.ready_event.set):
            ...
            if "h5" not in energy_scan_file_prefix:
                energy_scan_raw_file_name = os.path.extsep.join((energy_scan_file_prefix, "h5"))
                energy_scan_png_file_name = os.path.extsep.join((energy_scan_file_prefix, "png"))
            else:
            ...
            logging.getLogger("HWR").debug("energyScanFinished signal emitted %r", self.scanInfo)
    ...
    def store_energy_scan(self):
        ...
        if self.db_connection_hwobj:
            ...
            try:
                ...
                logging.getLogger("HWR").warning("Energy scan store in lims failed %s" %str(ex))
    ...
    def doChooch(self, elt, edge, scan_directory, archive_directory, prefix):
        logging.getLogger("HWR").info("Doing Chooch, scan directory %s, prefix  %s" %(scan_directory, prefix))
        ...
        try:
            pk, fppPeak, fpPeak, ip, fppInfl, fpInfl, chooch_graph_data = PyChooch.calc(zip(x_array, y_array), elt, edge, archive_file_efs_filename)
        ...
        try:
            ...
            logging.getLogger("HWR").info("Energy scan: saving chooch results into file %s" %data_filename)
            file = h5py.File(data_filename, 'a')
            entry = file.keys()[0]
            dset = file.create_dataset("/{}/results/peak_energy".format(entry), data=pk)
            dset.attrs['unit'] = 'keV'
            dset = file.create_dataset("/{}/results/inflection_energy".format(entry), data=ip)
            dset.attrs['unit'] = 'keV'
            dset = file.create_dataset("/{}/results/remote_energy".format(entry), data=rm)
            dset.attrs['unit'] = 'keV'
            file.close()
        except Exception as ex:
            logging.getLogger("HWR").error("Energy scan: cannot save chooch results to file: %s" %ex)
        ...
        try:
            ...
            logging.getLogger("HWR").info("Energy scan: saving chooch processing results into file %s" %data_filename)
            file = h5py.File(data_filename, 'a')
            entry = file.keys()[0]
            dset = file.create_dataset("/{}/results/chooch_x".format(entry), data=chooch_graph_x)
            dset.attrs['unit'] = 'eV'
            dset = file.create_dataset("/{}/results/chooch_y1".format(entry), data=chooch_graph_y1)
            dset.attrs['unit'] = ''
            dset = file.create_dataset("/{}/results/chooch_y2".format(entry), data=chooch_graph_y2)
            dset.attrs['unit'] = ''
            file.close()
        except Exception as ex:
            logging.getLogger("HWR").error("Energy scan: cannot save chooch processing results to file: %s" %ex)
    ...
    def open_safety_shutter(self):
        ...
        with gevent.Timeout(5, RuntimeError("Could not open the safety shutter, timeout error")):
