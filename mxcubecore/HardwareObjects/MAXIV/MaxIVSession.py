...
class MaxIVSession(Session):
    ...
    def _is_proposal_type(self, proposal_type, proposal_number=None):
        ...
        if proposal_number is not None:
            try:
                ...
                prop_type = duo.get_proposal(proposal_number).get(
                    "prop_type", "unknown"
                )
            except Exception as ex:
                logging.getLogger("HWR").warning(
                    "[MAX IV Session] cannot retrieve proposal info from DUO,  %s"
                    % str(ex)
                )
                return False
    ...
    def is_inhouse(self, proposal_code=None, proposal_number=None):
        ...
        logging.getLogger("HWR").debug(
            "[MAX IV Session] Checking if {} proposal is inhouse".format(
                proposal_number
            )
        )
    ...
    def is_proprietary(self, proposal_number=None):
        ...
        """
        logging.getLogger("HWR").debug(
            "[MAX IV Session] Checking if {} proposal is proprietary".format(
                proposal_number
            )
        )
